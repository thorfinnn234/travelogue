...
export default function CreateScreen() {
  ...
  const [image, setImage] = useState(""); // { uri, width, height }
  ...
  const askMediaPerms = async () => {
    ...
    if (!lib.granted) { Alert.alert("Permission needed", "Please allow photo library access."); return false; }
    return true;
  };

  const askCameraPerms = async () => {
    const cam = await ImagePicker.requestCameraPermissionsAsync();
    if (!cam.granted) { Alert.alert("Permission needed", "Please allow camera access."); return false; }
    return true;
  };

  const pickFromGallery = async () => {
    const ok = await askMediaPerms(); if (!ok) return;
    const res = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images,
      allowsEditing: true, aspect: [4, 5], quality: 0.9,
    });
    if (!res.canceled) {
  ...
  const takePhoto = async () => {
    const ok = await askCameraPerms(); if (!ok) return;
  ...
  const onSubmit = async () => {
    try {
      ...
      // If you have a backend, upload here (FormData). After the API responds, use its returned post.

      // Call the posts store using the store's expected shape. The store expects
      // an object with `imageUri`, `caption`, `location`, and optional user fields.
      addPost({
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        <Text style={[styles.h1, { color: fg }]}>Create Post</Text>

        {image ? (
          <View style={styles.previewWrap}>
            <Image source={image.uri} style={styles.preview} contentFit="cover" transition={200} />
        ...
        />
        <Text style={[styles.helper, { color: subtle }]}>{caption.length}/{MAX_CAPTION}</Text>
...
const styles = StyleSheet.create({
  ...
  drop: { borderWidth: 1, borderStyle: "dashed", borderRadius: 16, padding: 16, alignItems: "center", justifyContent: "center" },
  ...
  previewWrap: { borderRadius: 16, overflow: "hidden", backgroundColor: "#fff", borderWidth: 1, borderColor: "#e5e7eb" },
  ...
  submit: { marginTop: 20, borderRadius: 14, paddingVertical: 14, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 8 },
